...
namespace irods {
    ...
    void storage_tiering::migrate_violating_data_objects(
        ...
        try {
            ...
            for(const auto& list_itr : query_list) {
                const auto  query_type = query<rsComm_t>::convert_string_to_query_type(list_itr.second);
                const auto& query_string = list_itr.first;

                try {
                    query<rsComm_t> violating_query{comm_, query_string, query_limit, query_type};
                    rodsLog(
                        config_.data_transfer_log_level_value,
